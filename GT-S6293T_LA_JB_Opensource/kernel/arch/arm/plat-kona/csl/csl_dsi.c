...
void CSL_DSI_Reset(void)
{
	/* TODO change the hardcoded values to #defines
	0x3C000F00 - 			MM_RST (KONA_MM_RST_VA)
	KONA_MM_RST_VA - 		WR_ACCESS reg
	KONA_MM_RST_VA + 0x4	SOFT_RSTN0
	KONA_MM_RST_VA + 0x8	SOFT_RSTN1

	axipv_soft_rstn 		bit 0x100 in SOFT_RSTN0
	pixelv_soft_rstn		bit 0x10 in SOFT_RSTN1
	dsi0_soft_rstn			bit 0x2 in SOFT_RATN1
	*/

	/* Steps
	1. Enable write access in WR_ACCESS register
	2. Disable DSI, reenable it.
	(Wout the below two steps the image is shifted sometimes)
	3. Disable PV , reenable it.
	4. Disable AXIPV, reenable it.
	*/

	UInt32 regval, retry;

	pr_err("Resetting DSI \n");

	regval = readl(KONA_MM_RST_VA + 0x8);
	pr_err("regval  SOFT_RSTN1 is %x", regval);
	retry=0;
	do {
		writel(0x00A5A500, (KONA_MM_RST_VA + 0x0));
		writel(0x00A5A501, (KONA_MM_RST_VA + 0x0));
		udelay(100);
	writel(regval &  ~0x2, (KONA_MM_RST_VA + 0x8));
	msleep(100);
	regval = readl(KONA_MM_RST_VA + 0x8);
	pr_err("regval  SOFT_RSTN1 is %x", regval);
	} while ((regval & 0x2) &&(retry++<10));
	retry=0;
	do {
		writel(0x00A5A500, (KONA_MM_RST_VA + 0x0));
		writel(0x00A5A501, (KONA_MM_RST_VA + 0x0));
	...
	do {
		...
		writel(0x00A5A501, (KONA_MM_RST_VA + 0x0));
		udelay(100);
	writel(regval &  ~0x10, (KONA_MM_RST_VA + 0x8));
	msleep(100);
	regval = readl(KONA_MM_RST_VA + 0x8);
	pr_err("regval  SOFT_RSTN1 is %x", regval);
	} while ((regval & 0x10) &&(retry++<10));
	retry=0;
	do {
		writel(0x00A5A500, (KONA_MM_RST_VA + 0x0));
		writel(0x00A5A501, (KONA_MM_RST_VA + 0x0));
	...
	regval = readl(KONA_MM_RST_VA + 0x4);
	pr_err("regval  SOFT_RSTN0 is %x", regval);
	retry=0;
	do {
		writel(0x00A5A500, (KONA_MM_RST_VA + 0x0));
		writel(0x00A5A501, (KONA_MM_RST_VA + 0x0));
		udelay(100);
		writel(regval & ~0x100, (KONA_MM_RST_VA + 0x4));
		msleep(100);
		regval = readl(KONA_MM_RST_VA + 0x4);
		pr_err("regval  SOFT_RSTN0 is %x", regval);
	} while ((regval & 0x100) && (retry++<10));
	retry=0;
	do {
		writel(0x00A5A500, (KONA_MM_RST_VA + 0x0));
		writel(0x00A5A501, (KONA_MM_RST_VA + 0x0));
...
start_tx:
			...
			while (dsi_stat & CHAL_DSI_STAT_TXPKT1_BUSY) {
				udelay(WAIT_GENERAL_US);
				dsi_stat = chal_dsi_get_status(dsiH->chalH);
				if (g_display_enabled && ++lp_busy_cnt > 100)
					break;
			}
			if (!(dsi_stat & PKT1_STAT_MASK))
