...
Int32 DSI_Open(DISPDRV_HANDLE_T drvH)
{
	...
	regulator_enable(disp_reg); /* Turn ON disp_reg first */
	msleep(10);
#else
	if (g_display_enabled)
		...
		UInt8 reg, UInt8 *rxBuff, UInt8 buffLen)
{
	...
	if ((cslRes != CSL_LCD_OK) ||
		...
		DSI_ERR(
			...
			, reg);
		if (cslRes == CSL_LCD_INT_ERR)
			res = -2;  //-2 error means no BTA from LDI.
		else
		res = -1;
	} else {
	...
	DISPLAY_POWER_STATE_T state)
{
	...
	case CTRL_PWR_ON:
		...
		case STATE_PWR_OFF:
#ifndef CONFIG_LCD_HX8357_SUPPORT
