...
#ifdef CONFIG_LCD_HX8357_SUPPORT
	...
	if (!atomic_read(&fb->is_fb_registered)) {
		...
		if (!fb->display_info->vmode){
				...
				if (wait_for_completion_timeout(
					...
					msecs_to_jiffies(1000)) <= 0) //sec_mhkim 
		...
		if (fb->display_info->vmode) {
			...
			if (wait_for_completion_timeout(
			&fb->prev_buf_done_sem,	msecs_to_jiffies(1000)) <= 0) //sec_mhkim 
				pr_err("%s:%d timed out waiting for completion",
					__func__, __LINE__);
		}
	}
...
static int disable_display(struct kona_fb *fb)
{
	...
	if (g_kona_fb->is_display_found == 1)
...
static int ESD_disable_display(struct kona_fb *fb)
{
	int ret = 0;
	fb->display_ops->power_control(fb->display_hdl, CTRL_ESD_RECOVERY);
	fb->display_ops->close(fb->display_hdl);
	fb->display_ops->exit(fb->display_hdl);
	kona_clock_stop(fb);
	konafb_debug("kona display is disabled successfully\n");
...
//{{ Mark for GetLog
	...
	if (g_display_enabled) {
		gpio_set_value(BACKLIGHT_PIN,0); /* backlight off */
		mdelay(3);
	}
#else
	if (g_display_enabled)
...
static int __init kona_fb_init(void)
{
	...
	g_display_enabled = 1;
#if defined(CONFIG_LCD_HX8357_SUPPORT)
	strcpy(g_disp_str, "HX8357");
#else
...
void kona_fb_esd_hw_reset(void)
{
...
#if 0
	//kona_clock_start(g_kona_fb);
	konafb_error(" before power_control\n");
	if (fb->display_ops->power_control(fb->display_hdl,
				       CTRL_SCREEN_OFF))
		konafb_error("Failed to blank this display device!\n");
	disable_display(g_kona_fb);
	kona_clock_stop(g_kona_fb);
#else
	konafb_error("before ESD_Disable_disbpay\n");
	ESD_disable_display(g_kona_fb);
	//disable_display(g_kona_fb);
#endif
	konafb_info("turn on lcd\n");
