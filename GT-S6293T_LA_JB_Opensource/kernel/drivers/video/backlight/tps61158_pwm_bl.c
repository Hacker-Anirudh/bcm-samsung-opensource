...
#if defined(CONFIG_MACH_HAWAII_SS_ROYVEDTV_REV00)
static int backlight_pin = 24; //sec_mhkim 20131108 24
...
#elif defined(CONFIG_MACH_HAWAII_SS_ROYVEDTV_REV00)
struct brt_value brt_table[] = {
   { MIN_BRIGHTNESS_VALUE,  32 }, // Min pulse 32
   { 32,  31 },
   { 46,  30 },
...
#if defined(CONFIG_MACH_HAWAII_SS_ROYVEDTV_REV00)
struct mutex tps61158_mutex;
DEFINE_MUTEX(tps61158_mutex);
...
#if defined(CONFIG_MACH_HAWAII_SS_ROYVEDTV_REV00)
        mutex_lock(&tps61158_mutex);	
        gpio_set_value(backlight_pin,0);
        mdelay(3); 	
        mutex_unlock(&tps61158_mutex);	
#endif
	getnstimeofday(&ts);
