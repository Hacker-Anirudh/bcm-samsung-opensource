...
inline void do_attach_work(int32_t regIntFlag,int32_t regDev1,int32_t regDev2)
{
    ...
    printk("%s:regIntFlag = 0x%x, regDev1= 0x%x,regDev2= 0x%x\n",__func__,regIntFlag,regDev1, regDev2);
    ...
    if (regIntFlag&RT8973_INT_ADCCHG_MASK){
        if (pDrvData->operating_mode){ // Manual Switch
            switch(regADC){
                ...
                break;
                case 0x17:
		//2013.12.09 hs.yi@latinsystem for RT8973 L company cable detection issue. 										
                I2CWByte(RT8973_REG_MANUAL_SW1,0x24);
                I2CWByte(RT8973_REG_MANUAL_SW2,0x01);
                pDrvData->accessory_id = ID_CHARGER;
                if (platform_data.charger_callback)
...
static int __init rt8973_init(void)
{
	...
	if (use_muic_rt8973 == 0) {
		...
		#if defined(CONFIG_MACH_HAWAII_SS_KYLEVE_REV00) || defined(CONFIG_MACH_HAWAII_SS_ROYVEDTV_REV00)\
               || defined(CONFIG_MACH_HAWAII_SS_KYLEVESS_REV00) || defined(CONFIG_MACH_HAWAII_SS_JBTLP_REV00)
