...
#include "./ZinitixFW/zinitix_touch_zxt_firmware(ZI012209).h"
	...
	if (zinitix_bit_test(touch_dev->touch_info.status, BIT_ICON_EVENT)) {
		for (i = 0; i < touch_dev->cap_info.button_num; i++) {
			if (zinitix_bit_test(touch_dev->icon_event_reg,
			 ...
			 ts_read_data(touch_dev->client, ZINITIX_BTN_WIDTH + i, (u8*)(touch_dev->button_sensitivity+i), 2);
			}
		}
...
static irqreturn_t zinitix_touch_work(int irq, void *data)
{
	...
	if (!init_prio) {
	/* put the kthread to run on the CPU1 only.
	* This will be done only once when the thread starts. */
		set_cpus_allowed(current, cpumask_of_cpu(1));
		init_prio = 1;
	}
...
static bool ts_set_touchmode(u16 value){
	...
	if(value == TOUCH_DND_MODE) {
		...
		if (ts_write_reg(misc_touch_dev->client, ZINITIX_AFE_FREQUENCY, 64)!=I2C_SUCCESS)
	...
	if(value == TOUCH_SDND_MODE) {
		...
		if (ts_write_reg(misc_touch_dev->client, ZINITIX_DND_U_COUNT, U_cnt)!=I2C_SUCCESS)
			printk(KERN_INFO "[zinitix_touch] TEST Mode : Fail to set ZINITIX_DND_U_COUNT %d.\n", U_cnt);
		if (ts_write_reg(misc_touch_dev->client, ZINITIX_AFE_FREQUENCY, 32)!=I2C_SUCCESS)
...
/*	
	...
	up(&misc_touch_dev->work_proceedure_lock);
*/
	val = misc_touch_dev->button_sensitivity[0];
...
/*	
	...
	up(&misc_touch_dev->work_proceedure_lock);
*/
	val = misc_touch_dev->button_sensitivity[1];
