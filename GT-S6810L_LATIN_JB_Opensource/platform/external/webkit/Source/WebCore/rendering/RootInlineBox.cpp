...
#define THAI_VOWEL_HEIGHT_ADJUST 7
...
//SISO_HTMLComposer Start
    , m_lineHeightAdjustValue(THAI_VOWEL_HEIGHT_ADJUST)
    , m_lineHeightAdjust(false)
//SISO_HTMLComposer End
{
...
//SISO_HTMLComposer Start
        	    ...
        	    String str = String(characters);
                    ...
                    for(int i=0; i<length; i++){  
                        if(((int)characters[i] == THAI_DO_CHADA || (int)characters[i] == THAI_TO_PATAK)
                            && ((i+1) < length) && ((int)characters[i+1] == THAI_SARA_U || (int)characters[i+1] == THAI_SARA_UU)) {
                            setLineHeightAdjust(true);
                            break;
                        } 
                    }
                }					
    ...
    if(isLineHeightAdjust()){
        maxHeight += m_lineHeightAdjustValue ;
	    setLineTopBottomPositions(lineTop,lineBottom + m_lineHeightAdjustValue);
    }
    else{
        // To avoid Clipping issues in some Vietnamese and Latin Chars
        if(renderer() && renderer()->document() && renderer()->document()->settings() && 
            renderer()->document()->settings()->editableSupportEnabled()){
            setLineTopBottomPositions(lineTop-THAI_VOWEL_HEIGHT_ADJUST, lineBottom);
        }
        else{
