	...
	const v3d_job_post_t *user_job)
{
	...
	if (session == instance->job.issued.exclusive.owner)
		list = user_job->job_type == V3D_JOB_USER ? &instance->job.issued.exclusive.user : &instance->job.issued.exclusive.bin_render;
	else
		list = user_job->job_type == V3D_JOB_USER ? &instance->job.issued.user           : &instance->job.issued.bin_render;
