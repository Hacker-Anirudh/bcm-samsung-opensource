...
static int adaptername_probe(struct btd_adapter *adapter)
{
	...
	if (read_local_name(&bdaddr, name) < 0)
	{
		...
		// PSN Carrier's Request : BT name suould be 'Personal Samsung + Model_Name'
		char sales_code[4] = {0};
		property_get("ro.csc.sales_code", sales_code, "");
		if((strlen(sales_code)>0) && !strcasecmp(sales_code, "PSN"))
		{
			char tName[MAX_NAME_LENGTH+1];
			sprintf(tName, "%s %s", "Personal Samsung", name);
			strcpy(name, tName);
		}
	}
