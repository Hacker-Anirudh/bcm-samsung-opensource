    ...
    if (!paintInfo.rect.intersects(overflowRect)){
        ...
        if(SecNativeFeature::getInstance()->getEnableStatus(TAG_CSCFEATURE_FRAMEWORK_ENABLETHAIVIETRESHAPING) == true){
            if(overflowRect.width() == 0
                ...
                InlineBox* curr = firstLeafChild();
                if(curr && curr->isInlineTextBox() && curr->logicalWidth() == 0){
                    overflowRect.setWidth(1);
                    if(paintInfo.rect.intersects(overflowRect))
                        visibleLeaf = true;
                }
            }
    ...
    if ((!parent() && m_firstLine && styleToUse != renderer()->style()) || (parent() && renderer()->hasBoxDecorations())) {
        ...
        if (parent() && renderer()->style()->hasBorder()) {
            ...
            if (!hasBorderImage || (!prevLineBox() && !nextLineBox()))
                ...
                IntRect clipRect = clipRectForNinePieceImageStrip(this, borderImage, IntRect(tx, ty, w, h));
                context->save();
...
void InlineFlowBox::paintMask(PaintInfo& paintInfo, int tx, int ty)
{
    ...
    if (!prevLineBox() && !nextLineBox()) {
        ...
        int stripWidth = isHorizontal() ? totalLogicalWidth : w;
        int stripHeight = isHorizontal() ? h : totalLogicalWidth;
        IntRect clipRect = clipRectForNinePieceImageStrip(this, maskNinePieceImage, IntRect(tx, ty, w, h));
        paintInfo.context->save();
