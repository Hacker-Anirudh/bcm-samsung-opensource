...
#define ACCESSORY_INSERTION_SETTLE_TIME 	msecs_to_jiffies(770)
#else
...
#elif defined(CONFIG_MACH_RHEA_SS_ZANIN)
#define BASIC_HEADSET_DETECT_LEVEL_MIN  681
#else
#define BASIC_HEADSET_DETECT_LEVEL_MIN  955
#endif
#if defined(CONFIG_MACH_RHEA_SS_ZANIN)
#define BASIC_HEADSET_DETECT_LEVEL_MAX  2149
#else
...
#if 0 //defined(CONFIG_MACH_RHEA_SS_NEVISP) || defined(CONFIG_MACH_RHEA_SS_NEVIS) || defined(CONFIG_MACH_RHEA_SS_NEVISDS)
...
static int read_adc_for_accessory_detection(int hst)
{
	...
	*/
	...
	do{
		...
		else if((readl(mic_dev->aci_base + ACI_COMP_DOUT_OFFSET) & (1 << 3)) || mic_dev->recheck_jack)
		{
			if(mic_dev->recheck_jack == 1)
				mic_dev->recheck_jack = 0;
		...
		if((readl(mic_dev->aci_base + ACI_COMP_DOUT_OFFSET) & (1 << 3)) || mic_dev->recheck_jack)
		{
			if(mic_dev->recheck_jack == 1)
				mic_dev->recheck_jack = 0;
...
static int detect_hs_type(struct mic_t *mic_dev)
{
	...
	*/
	...
	/* Instead of quickly turning off the bias in case of
	 * Headphone, read the ADC value to 
	 * allow it to stabilize. Do not change the BIAS
	 * Values again during this time*/
/*	if(type == HEADPHONE)
	{
		int verify_adc_value = 6;
		do {
			int i;
			for (i=HEADPHONE;i<HEADSET;i++)
			type = read_adc_for_accessory_detection(i);	
			msleep(KEY_ENABLE_DELAY);
		}while( --verify_adc_value );
	} */
#endif
