...
static int s2200_ts_power(int on_off)
{
	...
	pr_info("%s: NEVISVESS TS power change to %d.\n", __func__, on_off);
	...
	retval = gpio_request(TOUCH_POWER_GPIO_PULLUP,"Touch_pullen");
	if (retval) {
		pr_err("%s: Failed to acquire power GPIO_PULLUP, code = %d.\n",
			 __func__, retval);
		return retval;
	}
	...
	if (on_off == TOUCH_ON) {
		...
		if (retval) {
			pr_err("%s: Failed to set power GPIO PULLUP to 1, code = %d.\n",
				__func__, retval);
			return retval;
		}
		...
		if (retval) {
			pr_err("%s: Failed to set power GPIO PULLUP to 0, code = %d.\n",
				__func__, retval);
			return retval;
		}
...
#include "board-rhea_ss_nevisvess-wifi.h"
...
static int tm2303_post_suspend(const void *pm_data) {
	pr_info("%s: RMI4 callback.\n", __func__);
	return s2200_ts_power(TOUCH_OFF);
}

static int tm2303_pre_resume(const void *pm_data) {
	pr_info("%s: RMI4 callback.\n", __func__);
...
#if defined(CONFIG_STEREO_SPEAKER)// NEVISVESS_AMP
 ...
 */
#ifdef CONFIG_LCD_NEVISVE_SMI_SUPPORT
struct kona_fb_platform_data konafb_devices[] __initdata = {
	{
		.dispdrv_name  = "Nevis LCD",
		...
		.parms = {
			.w0 = {
				.bits = {
					...
					.bus_width  = RHEA_BUS_WIDTH_18,
					.te_input   = RHEA_TE_IN_0_LCD,
					.col_mode_i = RHEA_CM_I_XRGB888,
					.col_mode_o = RHEA_CM_O_RGB666,
				},
...
MACHINE_START(RHEA, "rhea_ss_nevisvess")
	.map_io = board_map_io,
