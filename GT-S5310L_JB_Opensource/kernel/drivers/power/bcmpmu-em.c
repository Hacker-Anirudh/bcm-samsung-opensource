...
static int debug_mask = BCMPMU_PRINT_ERROR | BCMPMU_PRINT_FLOW | BCMPMU_PRINT_INIT;
...
static int update_batt_capacity(struct bcmpmu_em *pem, int *cap)
{
	...
	if (calibration == 0) {
		if ((pem->fg_temp_fact < 1000) && (pem->batt_temp <= 20))
			pem->fg_temp_fact = 1000;
	...
	if ((!is_charger_present(pem)) &&
		(pem->batt_volt <= 3400 && capacity <= 3) )
	{
		pr_em(INIT, "%s : emergency shutdown!! cap =%d, batt_volt= %d, \n", __func__, capacity, pem->batt_volt);
	    capacity--;
...
static unsigned char zero_cap_marker=0;
static void update_power_supply(struct bcmpmu_em *pem, int capacity)
{
	...
	if (pem->piggyback_chrg) {
 		...
 		if (pem->batt_volt <= 3400 && capacity <= 1)
		{
			if(capacity <= 1 )
			{
				pr_em(INIT, "%s : capacity is equal or under 1, notify 0 capacity under voltage %d\n", __func__, pem->batt_volt);
				capacity = (zero_cap_marker)?0:1;
				zero_cap_marker = 1;
			}
			else
			{
				pr_em(ERROR, "%s : abnormal capacity = %d, under voltage %d\n", capacity, pem->batt_volt);
			}
...
#ifdef CONFIG_KONA_PI_MGR
	pi_mgr_qos_add_request(&qos_node, "em_qos_node", PI_MGR_PI_ID_ARM_SUB_SYSTEM,
...
static int bcmpmu_em_resume(struct platform_device *pdev)
{
	...
	pr_em(INIT, "pem->voltage=%d, cutoff=%d\n", pem->batt_volt, pem->cutoff_volt);
	return 0;
