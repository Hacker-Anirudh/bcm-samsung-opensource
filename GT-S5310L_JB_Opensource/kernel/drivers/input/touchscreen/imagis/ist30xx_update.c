...
const u8 *ist30xx_fw = ist30xx_fw_pf;
const u8 *ist30xx_param = ist30xx_param_pf;
u32 ist30xx_fw_size = sizeof(ist30xx_fw_pf);
u32 ist30xx_param_size = sizeof(ist30xx_param_pf);
...
int ist30xx_isp_fw_read(struct i2c_client *client, u32 *buf32)
{
	...
	if (ret) {
		pr_err("[ TSP ] ISP cmd fail, CMD_ENTER_ISP_MODE\n");
		return ret;
	}
	...
	for (i = 0; i < IST30XX_EEPROM_SIZE; i += EEPROM_PAGE_SIZE) {
		...
		if (ret) {
			pr_err("[ TSP ] ISP cmd fail, CMD_ISP_START_READ\n");
			return ret;
		}

		for (j = 0; j < EEPROM_PAGE_SIZE / IST30XX_DATA_LEN; j += 4)
			printk("[ TSP ] %04x: %08x %08x %08x %08x\n",
			       i + j, buf32[j], buf32[j + 1], buf32[j + 2], buf32[j + 3]);
...
int ist30xx_isp_bootloader(struct i2c_client *client, const u8 *buf)
{
	...
	u8 dest_buf[EEPROM_PAGE_SIZE];

	u32 *src_buf32 = (u32 *)buf;
	u32 *dest_buf32 = (u32 *)dest_buf;

	printk("[ TSP ] *** Bootloader update (0x%04x~0x%04x) ***\n",
	...
	if (ret) {
		pr_err("[ TSP ] ISP cmd fail, CMD_ISP_ERASE_ALL_PAGE\n");
		goto recv_end;
	}
	...
	for (i = 0; i < page_cnt; i++) {
		...
		if (ret) {
			pr_err("[ TSP ] ISP cmd fail, CMD_ISP_PROGRAM_PAGE\n");
			goto recv_end;
		}
		msleep(2);
...
int ist30xx_get_tkey_info(void)
{
	...
	ist30xx_get_update_info(ts_data, ist30xx_fw, ist30xx_fw_size);
...
int ist30xx_fw_update(struct i2c_client *client, const u8 *buf, int size, bool mode)
{
	...
	if (ret) {
		printk("[ TSP ] F/W update Fail!, ret=%d", ret);
	} else if (chksum != ts_data->fw.chksum) {
		printk("[ TSP ] Error CheckSum: 0x%08x(0x%08x)\n",
...
int ist30xx_param_update(struct i2c_client *client, const u8 *buf, int size)
{
	...
	DMSG("[ TSP ] F/W version core: %08x, param: %08x\n",
            ts_data->fw.ver, ts_data->param_ver);
...
int ist30xx_auto_bin_update(struct ist30xx_data *data)
{
    ...
    ist30xx_get_update_info(data, ist30xx_fw, ist30xx_fw_size);
	...
	while (retry--) {
		...
		DMSG("[ TSP ] tsp type : %d\n", data->tsp_type);
        if (ret == 0) {
			if ((data->tsp_type == 0) || (data->tsp_type == 1))
			tsp_check = true;
			break;
	}
	}
	if (!tsp_check) {
		...
		while (retry--) {
			ret = ist30xx_isp_bootloader(data->client, ist30xx_fw);
			if (ret == 0) {
				ist30xx_reset();
				ret = ist30xx_read_cmd(data->client, CMD_GET_TSP_PANNEL_TYPE, &data->tsp_type);
				DMSG("[ TSP ] tsp type : %d\n", data->tsp_type);
...
#else
    ist30xx_read_cmd(data->client, CMD_GET_TSP_PANNEL_TYPE, &data->tsp_type);
    ...
    // for SEC MINT, CORSICA
	if (data->tsp_type /*ist30xx_tsp_type*/ == 0) {    // tsp_type : 0 --> PFF
		ist30xx_fw = ist30xx_fw_pff;
		ist30xx_param = ist30xx_param_pff;
		ist30xx_fw_size = sizeof(ist30xx_fw_pff);
		ist30xx_param_size = sizeof(ist30xx_param_pff);
	}
	DMSG("[ TSP ] tsp type : %d, size: %d\n", data->tsp_type /*ist30xx_tsp_type*/, ist30xx_fw_size);
	...
	if ((data->param_ver > 0) && (data->param_ver < 0xFFFFFFFF) &&
		...
		DMSG("[ TSP ] F/W: %08x(%08x), Param: %04x(%04x)\n",
	...
	DMSG("[ TSP ] Update version F/W: %08x -> %08x, Param: %04x -> %04x\n",
	     data->fw.ver, ist30xx_fw_ver, data->param_ver, ist30xx_param_ver);
	...
	while (retry--) {
		if (isp_mode)
			ret = ist30xx_fw_update(data->client,
						ist30xx_fw, ist30xx_fw_size, true);
		else
			   ...
			   const char *buf, size_t size)
{
	...
	if (mode == 3) {
		DMSG("[ TSP ] EEPROM all erase test\n");
			    ...
			    const char *buf, size_t size)
{
	...
	if (mode == MASK_UPDATE_BIN) {
		...
		param_size = (u32)ist30xx_param_size;
#else
		return size;
#endif          // IST30XX_INTERNAL_BIN
	}
...
/* sysfs  */
static DEVICE_ATTR(firmware, S_IRUGO, ist30xx_fw_status, ist30xx_fw_store);
static DEVICE_ATTR(fwparam, S_IRUGO, NULL, ist30xx_fwparam_store);
static DEVICE_ATTR(boot, S_IRUGO, ist30xx_fw_show, ist30xx_boot_store);
static DEVICE_ATTR(version, S_IRUGO, ist30xx_fw_version, NULL);
static DEVICE_ATTR(param, S_IRUGO, NULL, ist30xx_param_store);
