...
static int adapter_set_mode(struct btd_adapter *adapter, uint8_t mode)
{
	...
	DBG("Moonys adapter_set_mode mode is %d, discovery tiimeout", mode, adapter->discov_timeout);
			...
			DBusMessage *msg)
{
	...
	{
		DBG("Moonys set_mode, pending is not NULL. Already ready");
		return -EALREADY;
	}
	...
	if (!adapter->up && new_mode != MODE_OFF) {
		...
		if (err < 0)
		{
			DBG("Moonys set_powered TRUE returned err[%d]", err);
			return err;
	...
	if (adapter->up && new_mode == MODE_OFF) {
		...
		if (err < 0)
		{
			DBG("Moonys set_powered FALSE returned err[%d]", err);
			return err;
		}
		else if(err == 400)
		{
			DBG("Moonys set_powered FALSEreturned err[%d]", err);			
			msg = NULL;
		}
	...
	{
		DBG("Moonys set_powered new_mode IS same with adpater->mode");
		return 0;
	}
	...
	if (msg != NULL) {
		...
		if (req) {
			DBG("Moonys set_1111111111111111111");			
			adapter->pending_mode = req;
			session_ref(req);
		} else
			{
			/* Wait for mode change to reply */
						DBG("Moonys set_222222222222222222222222222");			
			adapter->pending_mode = create_session(adapter,
					connection, msg, new_mode, NULL);
			}
	} else
		{
					DBG("Moonys set_333333333333333333333333");			
				...
				gboolean discoverable, void *data)
{
	...
	DBG("Wanted mode is %d, current is %d",mode, adapter->mode);
