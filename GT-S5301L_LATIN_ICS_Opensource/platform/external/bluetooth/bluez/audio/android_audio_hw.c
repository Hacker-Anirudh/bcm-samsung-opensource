...
{
    const struct astream_out *out = (const struct astream_out *)stream;
    return out->buffer_size * (BUF_NUM_PERIODS / 2); 
}
...
static uint32_t out_get_latency(const struct audio_stream_out *stream)
{
	...
	latency_ms = out->buffer_duration_us / 1000; 
	latency_ms += (out->buf_size * 1000) / out->sample_rate; 
	latency_ms += 200;     
	
	LOGV("A2DP latency_ms = %d", latency_ms);  
   return latency_ms; 
#else 
