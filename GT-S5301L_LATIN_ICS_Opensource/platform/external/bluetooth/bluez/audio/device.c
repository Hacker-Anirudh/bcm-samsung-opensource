				...
				void *user_data)
{
	...
	case SINK_STATE_PLAYING:
        /* Headset role switch temporary patch for H3500 chopping problem when WIFI search AP */
        /* Remove Temporary patch because WIFI Firmeare update for COEX */
        /* 
        {
	    	int dd=0, ret=0;
	    	dd = hci_open_dev(0);
	   	 ...
	   	 if(dd >= 0 )
	    	{        
	      		ret = hci_switch_role(dd,&dev->dst,0,1000);
	     		hci_close_dev(dd);                                                    
	    	}
	}
	*/   
