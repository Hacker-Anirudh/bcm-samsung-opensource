...
/* BRCM FixMe: This is a temp fix to detect the BT/FM state so as to decide whether
* to inform the frameworks for quick-switch scenarios
*/
#include "../../../../system/bluetooth/bluedroid/include/bluedroid/chip_state.h"
...
static int hciops_power_off(int index)
{
	...
	property_get("service.brcm.bt.fm.active", value, "0");
	info("FM is active : service.brcm.bt.fm.active=%s\n", value);
	if (strcmp(value, "1") == 0) {
		return 0;
	}else {
...
/*Bluez will not work properly when connect&disconnect with headset very quickly, It looks like bluez kernel
 does not work properly when connect&disconnect quickly, so here disconnect all ACL connection intentional, to make the repetition rate much less.*/
#ifndef HCI_DEV_ID
#define HCI_DEV_ID  0
#endif
