 ...
 **/
bool isNotThaiTonehw(int32_t unicode) {
    ...
    || (unicode == SARA_AA)) {
#ifdef THAIRESHAPERLOGS
		LOGE("ThaiReshaper::isNotThaiTone, true");
#endif
		return true;
	}
#ifdef THAIRESHAPERLOGS
	LOGE("ThaiReshaper::isNotThaiTone, false");
#endif
	return false;
 ...
 **/
 //Main functionality taken from skia/src/core/ThaiReshaper.cpp
bool needsThaiReshaping(const UChar* utf, size_t len) {
    int consonanttype;
    for (size_t i = 0; i < len; i++) {
        if(isThaihw(utf[i])) {
            consonanttype = isThaiReshaperCharhw(utf[i]);
            if (consonanttype != NOTTHAIRESHAPE) {
                for (size_t j = i + 1; ((j <= (i + 3)) && (j < len)); j++) {
                    if (isNotThaiTonehw(utf[j])) {
                        if (consonanttype != THAIRESHAPESHIFTLEFT) {
#ifdef THAIRESHAPERLOGS
                            LOGE("ThaiReshaper::hasThai, break");
#endif
                            break;
                        } else {
                            if (utf[j] != SARA_AM) {
                    ...
                    if (isThaiReshaperTonehw(utf[j], consonanttype) == consonanttype) {
#ifdef THAIRESHAPERLOGS
                        LOGE("ThaiReshaper::hasThai, true..");
#endif
                        return true;
                    }
...
#ifdef THAIRESHAPERLOGS
    LOGE("ThaiReshaper::hasThai, false");
#endif
    return false;
}
 ...
 **/
bool thaiReshaperInPlace(UChar** outPtr, size_t inLen){
        UChar* string = *outPtr;
        int result = NOTTHAIRESHAPE;
        bool replaced = false;	
        for(size_t i=0; i<inLen; i++)
        {
                if(isThaihw(string[i])){
                        ...
                        switch(cT){
                                ...
                                case THAIRESHAPESHIFTLEFT: 
                                        { 
                                                bool isThirdCharacterIsThaiTone=!isNotThaiTonehw(string[i+2]);
                                                result=isThaiReshaperTonehw(string[++i],cT);
