...
float InlineTextBox::positionForOffset(int offset) const
{
    ...
    RenderText* text = toRenderText(renderer());
/*
    //SNMC_BROWSER_CURSOR_RELATED_CHANGES_FOR SARA_AM_THai_Tone(yogendra.s)_START 
            ...
            for(int i=m_start;i<oldOffset;i++){
                    if(character[i] == SARA_AM){
                            if(isThaihw(character[i-1])){
                                    ...
                                    bool isCharacterReplaced=isThaihw(character[i-2])&& !isNotThaiTonehw(character[i-1]);
...
*/
    //SNMC_BROWSER_CURSOR_RELATED_CHANGES_FOR SARA_AM_THai_Tone(yogendra.s)_END 
